...
// Following items undo their layering. Once. Bodypart has to be taken into account, hence the switch.
    switch (bp) {
    case bp_feet  :
        if (!(is_wearing(itm_socks) || is_wearing(itm_socks_wool))) break;
        else layers--;
    case bp_legs  :
        if (!is_wearing(itm_long_underpants)) break;
        else layers--;
    case bp_hands :
        if (!is_wearing(itm_gloves_liner)) break;
        else layers--;
    case bp_torso :
        if (!is_wearing(itm_under_armor)) break;
        else layers--;
    }
    if (layers > 1)
